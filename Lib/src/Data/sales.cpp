...
Sale::Sale(QString id, QString name, QString category, int quantity, int cost, int totalcost, QString Date, QString user, QString payment, int mProfit):
  _Id(id), _Name(name),_Category(category), _Quantity(quantity), _Cost(cost), _Total_cost(totalcost),date(Date),mUser(user),mPayment(payment),mProfit(mProfit)
{
...
int Sale::getProfit() const
{
    return mProfit;
}

void Sale::setProfit(int profit)
{
    if(mProfit != profit){
    mProfit = profit;
    emit profitChanged();
...
int Sale::getDiscount() const
{
    return mDiscount;
}

void Sale::setDiscount(int discount)
{
...
int Sale::getTotalDiscount() const
{
    return mTotalDiscount;
}

void Sale::setTotalDiscount(int totalDiscount)
{
    if( mTotalDiscount != totalDiscount){
